...
def getHumanReadableSize(size_in_bytes):
    """Return human-readable version of size_in_bytes.

    Suffix:
    X > 1024 * 1024 => M
    X > 1024 and X < 1024 * 1024 => K
    X < 1024 => No suffix
    """
    human_readable_size = size_in_bytes
    if human_readable_size >= 1048576:
        human_readable_size = str (human_readable_size / 1048576) + 'M'
    elif human_readable_size >= 1024:
        human_readable_size = str (human_readable_size / 1024) + "K"
    return str (human_readable_size)
...
class Cache (object):
    ...
    def getStats (self):
        """Return list of vital cache statistics.
        ...
        #capacity misses (???)
        """
        l1 = [self.getCumulativeCount (count)
                for count in ['access_count',
                              'cold_miss_count',
                              'conflict_miss_count']]
        l2 = [self.getHitRate ()]
    ...
    def getParameters(self):
        """Return list of parameters for this cache.
    ...
    def getHitRate(self):
        """Return the % Hit Rate for cache.
        """
        miss = (self.getCumulativeCount ('cold_miss_count')
                + self.getCumulativeCount ('conflict_miss_count'))
        total = self.getCumulativeCount ('access_count')
        return 100 * (1 - float (miss) / total)
